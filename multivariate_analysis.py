...
Designed for NON‑TECHNICAL users. Every chart has:
  ...
  - Axis labels written as questions or simple descriptions
...
def _sample(df: pd.DataFrame) -> pd.DataFrame:
    """Down‑sample large data frames for faster visualisation."""
    if len(df) > MAX_ROWS_SAMPLE:
        return df.sample(MAX_ROWS_SAMPLE, random_state=42).reset_index(drop=True)
    return df.copy()


def _numeric_cols(df: pd.DataFrame, min_unique: int = 5) -> list:
    """Return numeric columns that have at least `min_unique` distinct values."""
    return [
        c
...
def _cat_cols(df: pd.DataFrame, max_unique: int = 20) -> list:
    """Return categorical columns with at most `max_unique` distinct values."""
    return [
        c
...
def _style_ax(ax: plt.Axes) -> None:
    """Apply the Argus colour scheme to an Axes."""
...
def _insight_box(fig: plt.Figure, text: str, rect=(0.04, 0.03, 0.92, 0.18)) -> None:
    """Render the AI insight panel – identical look to the bivariate version."""
...
def _ask_ai(prompt: str, max_tokens: int = 130) -> str:
...
def _clean(df: pd.DataFrame, cols: list) -> pd.DataFrame:
    """Return a copy containing only `cols` with rows that have no NAs."""
...
# ═══════════════════════════════════════════════════════════════════════════════
def _build_profiles(df: pd.DataFrame) -> dict:
    """Create a concise JSON‑friendly profile for every column."""
    profiles = {}
...
def _ai_select_columns(
    df: pd.DataFrame,
    profiles: dict,
    target: str,
    dataset_name: str,
) -> list:
    """Ask the LLM to pick the most meaningful columns (fallback to variance)."""
    ...
    # Fallback – pick highest‑variance numeric columns
    return df[num_cols].var().nlargest(MAX_COLS).index.tolist()


# ═══════════════════════════════════════════════════════════════════════════════
# Page 1 — "Which things move together?" (relationship grid)
# ═══════════════════════════════════════════════════════════════════════════════
def _page_relationships(pdf, df, num_cols, dataset_name):
    ...
    fig, ax = plt.subplots(
        figsize=(fig_size, fig_size * 0.82), facecolor=BG_COLOR
    )
    ...
    # Identify strongest positive / negative links
...
# ═══════════════════════════════════════════════════════════════════════════════
# Page 2 — "What drives [target] the most?" (feature importances)
# ═══════════════════════════════════════════════════════════════════════════════
def _page_top_influencers(pdf, df, num_cols, target, dataset_name):
    ...
    is_regression = (
        pd.api.types.is_numeric_dtype(clean[target])
    ...
    fig, ax = plt.subplots(
        figsize=(10, max(4, len(labels) * 0.55 + 2)), facecolor=BG_COLOR
    )
    fig.subplots_adjust(bottom=0.38, left=0.02, right=0.88)

    bar_cols = [BAR_COLOR] + [LINE_COLOR] * (len(labels) - 1)
    bars = ax.barh(labels[::-1], pct[::-1], color=bar_cols[::-1], alpha=0.88, height=0.6)
    ...
    ax.set_xlabel(
        "How much influence does this column have?  (%)",
        fontsize=9,
        color=TEXT_COLOR,
    )
    ax.set_title(
        f"What drives  '{target}'  the most?",
        fontsize=13,
        fontweight="bold",
        color=BAR_COLOR,
        pad=10,
...
# ═══════════════════════════════════════════════════════════════════════════════
# Page 3 — "Are there natural groups?" (PCA + KMeans)
# ═══════════════════════════════════════════════════════════════════════════════
def _page_natural_groups(pdf, df, num_cols, target, dataset_name):
    ...
    # If a low‑cardinality categorical target exists, colour by it instead
    ...
    ax.set_xlabel(
        f"← Spread direction 1  ({ev[0]:.0%} of variation)",
        fontsize=8.5,
        color=TEXT_COLOR,
    )
    ax.set_ylabel(
        f"← Spread direction 2  ({ev[1]:.0%} of variation)",
        fontsize=8.5,
        color=TEXT_COLOR,
    )
    ax.set_title(
...
# ═══════════════════════════════════════════════════════════════════════════════
# Page 4 — "How does each group behave?" (profile cards)
# ═══════════════════════════════════════════════════════════════════════════════
def _page_group_profiles(pdf, df, num_cols, dataset_name):
    ...
    fig, axes = plt.subplots(
        1, k, figsize=(3.5 * k, 6.5), facecolor=BG_COLOR, sharey=True
    )
    ...
    for ci, ax in enumerate(axes):
        ...
        ax.set_title(
            f"{GROUP_EMOJIS[ci]}  Group {ci + 1}\n({n_rows} rows)",
            fontsize=10,
            fontweight="bold",
            color=colour,
            pad=6,
        )
        ax.set_xlabel("← Below avg  |  Above avg →", fontsize=7, color=TEXT_COLOR)
        _style_ax(ax)
        if ci == 0:
    ...
    # ------------------------------------------------------------------
    # FIXED LOGIC – SELECT THE MOST DIFFERENT COLUMN
    # ------------------------------------------------------------------
    # The original code had a stray parenthesis which raised a SyntaxError.
    # The intention is to pick the column whose *average absolute z‑score*
    # across the groups is the highest – i.e. the column that most
    # distinguishes the groups from the overall average.
    # ------------------------------------------------------------------
...
# ═══════════════════════════════════════════════════════════════════════════════
# Page 5 — "How are all columns connected?" (bubble connection map)
# ═══════════════════════════════════════════════════════════════════════════════
def _page_connection_map(pdf, df, num_cols, dataset_name):
    ...
    # Expand axis limits to make room for long column names (labels are placed at R * 1.22)
    ax.set_xlim(-1.5, 1.5)
    ax.set_ylim(-1.5, 1.5)

    # Edges – draw only if |corr| > 0.25
    for i in range(n):
...
def multi_visualize_analyze(
    ...
    # Ensure the target is always present
    if target_variable and target_variable in df_s.columns:
        if target_variable not in num_cols + cat_cols:
    ...
    with PdfPages(buf) as pdf:
        ...
        # ── Story pages ────────────────────────────────────────────────────────
        pages = [
            (_page_relationships, dict(num_cols=num_cols, dataset_name=dataset_name)),
            (
                _page_top_influencers,
                dict(num_cols=num_cols, target=target_variable, dataset_name=dataset_name),
            ),
            (
                _page_natural_groups,
                dict(num_cols=num_cols, target=target_variable, dataset_name=dataset_name),
            ),
            (_page_group_profiles, dict(num_cols=num_cols, dataset_name=dataset_name)),
            (_page_connection_map, dict(num_cols=num_cols, dataset_name=dataset_name)),
        ]
    ...
    buf.seek(0)
    pdf_bytes = buf.read()
    print("  Multivariate visualisation complete (memory‑resident).")
